...
export default function ComposeCampaign() {
  ...
  const recipientConfigs: Array<{
    ...
    },
    {
      ...
      label: "BCC",
      selected: bccSel,
      setSelected: setBccSel,
      extra: bccExtra,
      setExtra: setBccExtra,
    },
  ];
  ...
  function commitQuickAdd() {
    ...
    if (!existing.includes(email)) {
      const next = existing.length
        ? `${activeConfig.extra.replace(/\s+$/, "")}, ${email}`
        : email;
    ...
    <div className="container mx-auto p-6 max-w-6xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-slate-500">
          Choose recipients from your validated list, optionally include risky
          contacts, and craft your message.
        </p>
      </div>
      ...
      >
        <section className="card p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <div
              className="flex gap-4 p-2 bg-white rounded-xl shadow-sm justify-center"
              style={{ border: "1px solid rgb(59 130 246 / 0.25)" }}
            >
        ...
        <section className="card p-6 space-y-5">
          ...
          {riskView === "risky" && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-xs text-amber-700">
              Use caution—risky contacts are unverifiable or from accept-all
              domains. Monitor bounces before promoting them to valid.
            </div>
          )}
          ...
          <div className="space-y-1">
            ...
            <div className="text-xs text-slate-400">
              {extraCount} extra address{extraCount === 1 ? "" : "es"} will be
              included
            </div>
          </div>
        </section>
        ...
        <div className="lg:col-span-2 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          {err ? (
            ...
            <div className="text-sm text-slate-500">
              Total recipients (selected + extras):{" "}
